...
把自己的工作判断和表达习惯沉淀成可复用、可修改的 skills。每个 skill 在 `skills/` 下拥有独立目录，目前包括工作汇报 `report` 和专业电话会纪要 `call-notes`。

## 当前状态

`report` v0.6：已吸收 5 条历史素材和 4 条用户实战修订，维护 9 条主规则。本轮校准 boss recap：按本次要求组织任务与执行路径，向老师确认整体思路，并说明自己同步取材及继续推进的范围。给 boss、同事的消息及日报仍不加粗，便于直接复制到微信。原始材料只保存在本地，skill 包含必要的脱敏证据；尚无独立的个人风格相似度或可直接发送比例数据。

这里的“蒸馏”指从案例与反馈中提炼显式规则和精选例子，不涉及训练或修改模型权重。规则必须能追溯到样本或用户的明确说明。

`call-notes` v0.1：把陌生专业领域的录音、转写或笔记整理为紧凑的会议纪要，重点处理业务主线、关键条件、进展与相关行业信息，并区分会议说法、背景解释和推断。初版来自一个用户最终交付案例，等待下一例校准；尚未验证真实录音到满意终稿的完整效果。入口见 [专业电话会纪要](skills/call-notes/SKILL.md)。

调用方式：提供录音及已知背景，写“使用 $call-notes，帮我整理这次会议纪要”。转写依赖当前环境可用的音频能力，skill 本身不包含语音模型。
...
skills/call-notes/            专业电话会纪要及轻量校准说明
...
仓库当前为公开仓库；原始工作材料默认保存在被忽略的 `data-local/` 下。模板、通用方法和合成测试可以入库。真实样本只在完成脱敏、明确授权该批内容可公开后，才作为精选案例加入 skill。
